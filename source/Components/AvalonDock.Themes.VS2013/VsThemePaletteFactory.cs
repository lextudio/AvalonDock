...
// Reads a .vstheme (GZIP-compressed XML) palette file and builds a WPF
// ResourceDictionary of SolidColorBrush entries keyed by ComponentResourceKey.
//
...
{
    /// <summary>
    /// Reads a GZIP-compressed .vstheme XML file and builds a
    /// <see cref="ResourceDictionary"/> of <see cref="SolidColorBrush"/> entries
    /// keyed by <see cref="ResourceKeys"/> <see cref="ComponentResourceKey"/> values.
    /// </summary>
    internal sealed class VsThemePaletteFactory
    {
        ...
        {
            var src = (string)_env.Elements("Color")
                .FirstOrDefault(c => (string)c.Attribute("Name") == name)
                ?.Element("Background")?.Attribute("Source");
